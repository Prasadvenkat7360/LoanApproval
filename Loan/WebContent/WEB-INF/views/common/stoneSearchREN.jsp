				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							...
							<div class="col-md-8">								
								<select	id="stoneSuppBy" name="stoneSuppBy" disabled class="form-control">
									<c:forEach var="supBy" items="${suppliedBy}">
										<option selected="selected" value="${supBy.id}">${supBy.name}</option>
									</c:forEach>
								</select>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-8">
								<select name="segment" disabled id="segmentName"	class="form-control selecter" required>
									<c:if test="${not empty forSeg}">
										<option selected value="${forSeg.id}">${forSeg.description}</option>
									</c:if>
								</select> 
							</div>
						</div>
...
<script type="text/javascript" src='<c:out value="${pageContext.servletContext.contextPath}" />/resource/oe/assets/js/app/stoneSearchREN.js'></script>
