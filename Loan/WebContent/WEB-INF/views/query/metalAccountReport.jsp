...
<script type="text/javascript"
	src='resource/oe/assets/js/app/metalAccountReport.js'>
				...
				<form class="form-horizontal" id="metalAccSearch">
					<div class="mobile-responsive">
						<div class="row">
							...
							<div class="col-xs-12 col-sm-6 col-md-6 col-lg-2 form-field">
								<span class="required">*</span> <label>Store or DC</label> <select
									id="storeOrDC" class="form-control">
							...
							<div class="col-xs-12 col-sm-6 col-md-6 col-lg-3 form-field">
								<span class="required">*</span> <label>Name</label> <select
									id="storeOrDcId" class="form-control">
									<option value="" label="--Select--" />
								</select>
							</div>
						</div>
						<div class="row">
							...
							<div class="col-xs-12 col-sm-6 col-md-6 col-lg-4 form-field">
								<label>Document Type </label> <input type="text"
									class="form-control" name="docType" id="docType"
									style="text-transform: uppercase" />
							</div>
							<div class="col-xs-12 col-sm-6 col-md-6 col-lg-4 form-field">
...
<script type="text/javascript">
				...
				if (id != "") {
					$.getJSON(
							'/OrderExecution/api/v1/metalAccountLOV?page=storeOrDc&sdc='
								...
								//iterate over the data and append a select option
								$.each(data.payload.allStores, function(key,
										val) {
									$('#storeOrDcId').append(
											'<option value="' + val.id + '">'
													+ val.name + '</option>');
	...
	$("#Search")
			.on(
					...
					function() {
						if (validateReqd()) {
							postJSON("/OrderExecution/api/v1/generateMetalAccountRpt",JSON.stringify(metalAccountFilterValues()),function(response){
								if(typeof response != "undefined" && response.resCode == "1"){
									
									metalAccountReportGrid(response.payload.list);
									$("#jqxgrid").show();
								}
							});	
						} else {
							$.growl
									.error({
										message : "Please provide values for all the mandatory fields.",
										duration : 10000,
										title : 'Error'
									});
						}
