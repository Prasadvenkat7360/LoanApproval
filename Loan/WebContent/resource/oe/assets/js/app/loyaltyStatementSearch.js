...
var onLoadFunction = function(){
	...
	$.getJSON('/OrderExecution/api/v1/createLoyaltyStmntOnloadLOvs', function(data) {
		if(data.resCode == 1){
		$('#custTypeC').empty().append('<option value="" selected>--Select--</option>');
		$.each(data.payload.customer,	function(k, v) {
			$('#custTypeC').append('<option value="' + v.id + '">' + v.name + '</option>');
		});
		}
	});	
...
$("#clearAll").on('click',function(){
	window.location.href="javascript:showContentPage('loyaltyStatementSearch', 'bodySwitcher')"
});