...
var onLoadFunction = function(){
	...
	$.getJSON('/OrderExecution/api/v1/psrCancelLOVs', function(data) {
		...
		var v = '<select id="vendorCodeObj" name="vendorCodeObj" class="form-control"> <option value="" selected>--Select--</option>';
...
function loyaltySearchGrid() {
	...
	var datafields = [ 
		...
		{'name' : 'vendorName','type' : 'string','map' : 'vendorName'},

		{'name' : 'irType','type' : 'string','map' : 'irType'},

      ];

	var columns = [
		{'text' : 'From Date','datafield' : 'fromDate','width' : '25%',cellsalign : 'center',align : 'center',editable : false,sortable : false,groupable : true},
		{'text' : 'To Date ','datafield' : 'toDate','width' : '25%',editable : false,cellsalign : 'center',align : 'center',sortable : false},
		{'text' : 'Vendor Code','datafield' : 'vendorName','width' : '25%',editable : false,sortable : false,cellsalign : 'center',align : 'center'}, 

		{'text' : 'Reminder / Intimation','datafield' : 'irType','width' : '26%',editable : false,sortable : false,cellsalign : 'center',align : 'center'}, 

	];
	showMyGrid(datafields, "/OrderExecution/api/v1/searchVendorReminder","list", columns,loyaltyStatementFieldFilters(), updateRows, "");
	$("#jqxgrid").jqxGrid({
		width : '100%',
		columnsheight: 80,
        sortable: true,            
...
$("#clearAll").on('click',function(){
	window.location.href="javascript:showContentPage('vendorReminderSearch', 'bodySwitcher')"
});